...
Page({
  ...
  formSubmit: function (e) {
    ...
    if(that.data.needCapt && capt.length === 0){
      login.show('请填写图形验证码');
      return;
    }
    if(email.length === 0){
    ...
    if(!that.data.needCapt){
      ajax({
        url: 'user/yjs/checkcaptcha',
        data: {student_num: id},
        success: res => {
          if(res.data && res.data.errcode == 0){
            that.setData({
        ...
        if (res2.code) {
          ajax({
            ...
            success: res1 => {
              if (res1.data.errcode === 0) {
                wx.getUserInfo({
                  success: function(res){
                    ajax({
                      url: 'user/info/add/yjjw',
                      data: {
                        phone,
                        code,
                        student_num: id,
                        password: pass,
                        cookie: that.data.needCapt ? that.data.cookie : null,
                        cookie_code: capt ? capt : null,
                        iv: res.iv,
                      ...
                      success: res => {
                        ...
                        else if(res.data && res.data.errcode === 20015){
                          login.show("密码或者学号错误");
                        }
              ...
              }else{login.show(res1.data.errmsg);}
            },
          })
        } else {
  ...
  changecapt: function(e){
    var that = this;
    ajax({
      url: 'user/yjs/checkcaptcha',
      data: {student_num: that.data.id},
      success: res => {
        if(res.data && res.data.errcode == 0){
          that.setData({
            needCapt: false,
